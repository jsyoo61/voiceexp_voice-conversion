...
def train(train_A_dir, train_B_dir, model_dir, model_name, random_seed, validation_A_dir, validation_B_dir, output_dir, tensorboard_log_dir):
    ...
    sampling_rate = 16000 #논문은 20kHz, but 사용한 dataset이 다름
    num_mcep = 49 #higher order 24, lower  order 25
    frame_period = 5.0
    n_frames = 128
    lambda_cycle = 10
    ...
    print('load_wavs')
    # print('wav_A shape:%s, wav_B shape:%s'%(wavs_A[0].shape,wavs_B[0].shape))
    ...
    # print('coded_sps_A shape: %s, coded_sps_B shape: %s'%(coded_sps_A[0].shape, coded_sps_B[0].shape))
    ...
    coded_sps_A_lower_order, coded_sps_A_higher_order = separate_mceps(coded_sps_A_transposed)
    coded_sps_B_lower_order, coded_sps_B_higher_order = separate_mceps(coded_sps_B_transposed)

    coded_sps_A_lower_order_with_derivatives = differentiate_twice_and_combine(coded_sps_A_lower_order)
    coded_sps_B_lower_order_with_derivatives = differentiate_twice_and_combine(coded_sps_B_lower_order)

    coded_sps_A_norm, coded_sps_A_mean, coded_sps_A_std = coded_sps_normalization_fit_transoform(coded_sps = coded_sps_A_lower_order_with_derivatives)
    coded_sps_B_norm, coded_sps_B_mean, coded_sps_B_std = coded_sps_normalization_fit_transoform(coded_sps = coded_sps_B_lower_order_with_derivatives)
    ...
    model = CycleGAN(num_features = 75)
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(n_samples // mini_batch_size):
            ...
            generator_loss, discriminator_loss = model.train(input_A = dataset_A[start:end], input_B = dataset_B[start:end], lambda_cycle = lambda_cycle, generator_learning_rate = generator_learning_rate, discriminator_learning_rate = discriminator_learning_rate)
        ...
        if validation_A_dir is not None:
            if epoch % 50 == 0:
                ...
                for file in os.listdir(validation_A_dir):
                    ...
                    coded_sp_lower_order, coded_sp_higher_order = separate_mceps([coded_sp_transposed])
                    coded_sp_lower_order_with_derivatives = differentiate_twice_and_combine(coded_sp_lower_order)[0]
                    coded_sp_norm = (coded_sp_lower_order_with_derivatives - coded_sps_A_mean) / coded_sps_A_std
                    coded_sp_lower_order_converted_norm = model.test(inputs = np.array([coded_sp_norm]), direction = 'A2B')[0]
                    coded_sp_lower_order_converted = coded_sp_lower_order_converted_norm * coded_sps_B_std + coded_sps_B_mean
                    coded_sp_converted = np.concatenate([coded_sp_lower_order_converted[0:25], coded_sp_higher_order[0]], axis=0)
        ...
        if validation_B_dir is not None:
            if epoch % 50 == 0:
                ...
                for file in os.listdir(validation_B_dir):
                    ...
                    coded_sp_lower_order, coded_sp_higher_order = separate_mceps([coded_sp_transposed])
                    coded_sp_lower_order_with_derivatives = differentiate_twice_and_combine(coded_sp_lower_order)[0]
                    coded_sp_norm = (coded_sp_lower_order_with_derivatives - coded_sps_B_mean) / coded_sps_B_std
                    coded_sp_lower_order_converted_norm = model.test(inputs = np.array([coded_sp_norm]), direction = 'B2A')[0]
                    coded_sp_lower_order_converted = coded_sp_lower_order_converted_norm * coded_sps_A_std + coded_sps_A_mean
                    coded_sp_converted = np.concatenate([coded_sp_lower_order_converted[0:25], coded_sp_higher_order[0]], axis=0)
                    coded_sp_converted = coded_sp_converted.T
                    coded_sp_converted = np.ascontiguousarray(coded_sp_converted)
                    decoded_sp_converted = world_decode_spectral_envelop(coded_sp = coded_sp_converted, fs = sampling_rate)
...
if __name__ == '__main__':
    ...
    train_A_dir_default = '../Voice_Converter_CycleGAN-master/data/vcc2016_training/SF1'
    train_B_dir_default = '../Voice_Converter_CycleGAN-master/data/vcc2016_training/TM3'
    model_dir_default = './model/sf1_tm3'
    model_name_default = 'sf1_tm3.ckpt'
    random_seed_default = 0
    validation_A_dir_default = '../Voice_Converter_CycleGAN-master/data/evaluation_all/SF1'
    validation_B_dir_default = '../Voice_Converter_CycleGAN-master/data/evaluation_all/TM3'
