...
def separate_mceps(coded_sps):

    # lower order: 25, higher order: 24
    coded_sps_lower_order = list()
    coded_sps_higher_order = list()

    for coded_sp in coded_sps:
        coded_sps_lower_order.append(coded_sp[:25])
        coded_sps_higher_order.append(coded_sp[25:])

    return coded_sps_lower_order, coded_sps_higher_order

def differentiate_twice_and_combine(coded_sps):

    coded_sps_with_derivatives = list()

    for coded_sp in coded_sps:
        derivative_1st = np.gradient(coded_sp, axis=1)
        derivative_2nd = np.gradient(derivative_1st, axis=1)

        coded_sps_with_derivatives.append(np.concatenate([coded_sp, derivative_1st, derivative_2nd], axis=0))

    return coded_sps_with_derivatives
