...
def conversion(model_dir, model_name, data_dir, conversion_direction, output_dir):
    ...
    for file in os.listdir(data_dir):
        ...
        if conversion_direction == 'A2B':
            ...
            #f0_converted = f0
            coded_sp_norm = (coded_sp_lower_order_with_derivatives - mcep_mean_A) / mcep_std_A
            coded_sp_lower_order_converted_norm = model.test(inputs = np.array([coded_sp_norm]), direction = conversion_direction)[0]
            coded_sp_converted = coded_sp_lower_order_converted_norm * mcep_std_B + mcep_mean_B
        else:
            ...
            #f0_converted = f0
            coded_sp_norm = (coded_sp_lower_order_with_derivatives - mcep_mean_B) / mcep_std_B
            coded_sp_lower_order_converted_norm = model.test(inputs = np.array([coded_sp_norm]), direction = conversion_direction)[0]
            coded_sp_converted = coded_sp_lower_order_converted_norm * mcep_std_A + mcep_mean_A
